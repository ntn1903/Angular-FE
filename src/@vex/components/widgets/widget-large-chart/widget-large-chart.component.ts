...
export class WidgetLargeChartComponent implements OnInit {
  ...
  @Input() options: ApexOptions = defaultChartOptions({
    grid: { show: true, strokeDashArray: 3, padding: { left: 16 } },
    chart: { type: 'area', height: 384, sparkline: { enabled: false }, zoom: { enabled: false } },
    fill: { type: 'gradient', gradient: { shadeIntensity: 0.9, opacityFrom: 0.7, opacityTo: 0.5, stops: [0, 90, 100] } },
    colors: ['#C8ACD6', '#433D8B', '#2E236C','#17153B'],
    labels: createDateArray(12),
    xaxis: { type: 'datetime', labels: { show: true }, },
    yaxis: { labels: { show: true } },
    legend: { show: true, itemMargin: { horizontal: 4, vertical: 4 } },
  });
